        ...
        if (FramesRx > RxProcessed) {
            ...
            if (XEmacPs_BdIsRxNew(bdPtr)) {
                ...
                if (ethHdrPtr->frame_type == htons(0x0808)) {
                    ...
                    xil_printf("DST MAC: %02x:%02x:%02x:%02x:%02x:%02x\n\r", ethHdrPtr->dest_addr[0], ethHdrPtr->dest_addr[1], ethHdrPtr->dest_addr[2], ethHdrPtr->dest_addr[3], ethHdrPtr->dest_addr[4], ethHdrPtr->dest_addr[5]);
                    // sendPingReply(macPtr);
                }
